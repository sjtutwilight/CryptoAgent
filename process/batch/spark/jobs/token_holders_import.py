...
def parse_args():
    ...
    parser.add_argument(
        '--input-path',
        required=True,
        help='输入 JSON 文件路径或目录 (例如: runtime/data/dune/token-holders/1/0x514910771af9ca656af840dff83e8264ecf986ca/)'
    )
    parser.add_argument(
        '--warehouse',
...
def extract_metadata_from_path(input_path):
    ...
    # 路径格式: runtime/data/dune/token-holders/{chain_id}/{address}/
...
def create_database_and_table(spark, database, table_name):
    ...
    # 创建表 (如果不存在)
    # 注意: Paimon Spark 不支持 PRIMARY KEY 语法，使用 TBLPROPERTIES 的 primary-key 代替
    create_table_sql = f"""
    ...
    TBLPROPERTIES (
        'primary-key' = 'chain_id,token_address,wallet_address,snapshot_date',
        'bucket' = '4',
...
def process_data(spark, args):
    ...
    # 使用 insertInto 而不是 save，因为表已经存在
    df_transformed.write \
        .format("paimon") \
        .mode("append") \
        .insertInto(f"paimon.{args.database}.{args.table}")
