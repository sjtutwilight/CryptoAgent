...
- 2025-12-23: 增加 tool/ops.sh 作为数据初始化入口，包含 Postgres 维表与 Spark->Paimon 导入命令
- 2025-12-23: ops init:schema，一键初始化 Postgres/ClickHouse/Kafka schema
...
- 2025-12-23: 完成 Token Holders 数据通路调通
  ...
  - 数据上传：手动上传至 MinIO s3a://paimon-warehouse/test-data/
  - 数据导入：process/batch/spark/jobs/token_holders_import.py 通过 spark-submit 读取 JSON 写入 Paimon
