...
USE demo_db;
-- 2. 定义 Datagen 源表：DEX 事件（使用 BIGINT 时间戳）
...
-- 3. 定义 Datagen 源表：CEX Kline（使用 BIGINT 时间戳）
...
-- 0) Catalog & DB (self-contained so `-f` works in a fresh session)
...
CREATE DATABASE IF NOT EXISTS demo_db;
USE demo_db;
-- 4. 创建 Paimon Sink 表：无键日志表（DEX 事件）
...
-- 5. 创建 Paimon Sink 表：键表（CEX Kline）
