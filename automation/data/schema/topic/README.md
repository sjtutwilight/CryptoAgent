...
- `init_topics.sh`: Creates all topics from the list (idempotent with `--if-not-exists`).

## Usage
```bash
./automation/data/schema/topic/init_topics.sh
```

Optional environment overrides:
```bash
TOPIC_PARTITIONS=3 TOPIC_REPLICATION_FACTOR=1 ./automation/data/schema/topic/init_topics.sh
TOPIC_FILE=./automation/data/schema/topic/topics.list DRY_RUN=true ./automation/data/schema/topic/init_topics.sh
```
