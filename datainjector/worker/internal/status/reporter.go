...
func Init(cfg config.StatusReporterConfig) {
	globalReporterOnce.Do(func() {
		if !cfg.Enabled {
			log.Printf("[status] reporter disabled")
			return
		}
		if len(cfg.Brokers) == 0 {
			log.Printf("[status] reporter enabled but no brokers configured, disabling")
			return
		}
		if cfg.Topic == "" {
		...
		log.Printf("[status] reporter initialized: topic=%s brokers=%v", cfg.Topic, cfg.Brokers)
	})
}
...
func Close() {
	if r, ok := globalReporter.(*kafkaReporter); ok {
		if err := r.Close(); err != nil {
			log.Printf("[status] reporter close error: %v", err)
		}
	}
}
