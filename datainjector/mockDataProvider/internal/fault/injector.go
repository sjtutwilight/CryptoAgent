...
type FaultInjector struct {
	...
	wsDisconnectionCounter int64
	wsDataLossCounter      int64
}
...
func (f *FaultInjector) ShouldInjectWebSocketDisconnection() bool {
	...
	if everyN := f.config.Fault.WebSocket.DisconnectionEveryN; everyN > 0 {
		f.wsDisconnectionCounter++
		if f.wsDisconnectionCounter%int64(everyN) == 0 {
...
func (f *FaultInjector) ShouldInjectWebSocketDataLoss() bool {
	...
	if everyN := f.config.Fault.WebSocket.DataLossEveryN; everyN > 0 {
		f.wsDataLossCounter++
		if f.wsDataLossCounter%int64(everyN) == 0 {
...
func (f *FaultInjector) ResetStats() {
	...
	f.wsDisconnectionCounter = 0
	f.wsDataLossCounter = 0
}
