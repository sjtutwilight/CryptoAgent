...
import com.crypto.control.dto.HttpTaskMessage;
...
public class TimerProducerService {
    ...
    private void sendTaskToKafka(Task task) {
        try {
            // 转换为HTTP任务消息格式
            HttpTaskMessage httpTaskMessage = HttpTaskMessage.fromTask(task);
            
            // 序列化为JSON
            String messageJson = objectMapper.writeValueAsString(httpTaskMessage);
            
            // 详细日志输出，用于调试消息格式
            log.info("发送任务到Kafka: taskId={}, 原始Task参数={}, 转换后消息={}", 
                    task.getTaskId(), task.getParams(), messageJson);
