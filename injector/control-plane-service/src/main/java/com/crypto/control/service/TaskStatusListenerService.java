...
public class TaskStatusListenerService {
    ...
    private void handleRetry(Task task, TaskStatusUpdate statusUpdate) {
        if (task.getRetryCount() < task.getMaxRetryCount()) {
            ...
            // 计算重试延迟时间（指数退避）
            long delaySeconds = calculateRetryDelay(task.getRetryCount());
            task.setScheduledTime(LocalDateTime.now().plusSeconds(delaySeconds));
            
            log.info("任务将重试: taskId={}, retryCount={}, nextScheduledTime={}", 
                    task.getTaskId(), task.getRetryCount(), task.getScheduledTime());
        } else {
            // 达到最大重试次数，标记为最终失败
            task.setStatus(Task.TaskStatus.FAILED);
            task.setCompletedAt(LocalDateTime.now());
        ...
        TaskCreateRequest taskCreateRequest = TaskCreateRequest.builder()
                ...
                .build();
        mainProcessorService.processTask(taskCreateRequest);
    }
    /**
     * 处理任务开始执行
     */
