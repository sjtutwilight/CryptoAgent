...
func NewApplication(cfg *config.Config, logger *logrus.Logger) (*Application, error) {
	...
	// 创建Binance客户端
	binanceClient := client.NewBinanceClient(cfg, logger, dataProducer)

	// 创建QuickNode客户端
	quicknodeClient := client.NewQuicknodeClient(cfg, logger, dataProducer)
...
func (app *Application) Start(ctx context.Context) error {
	...
	// 启动Binance客户端
	if app.config.Websocket.Binance.Enable {
		app.wg.Add(1)
		go func() {
			defer app.wg.Done()
	...
	// 启动QuickNode客户端
	if app.config.Websocket.Quicknode.Enable {
		app.wg.Add(1)
		go func() {
			defer app.wg.Done()
